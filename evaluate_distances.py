...
model_types = ['bert-base-cased', 'bert-large-cased', 'gpt2', 'gpt2-medium', 'roberta-base', 'roberta-large']
rem_model_types = ['xlnet-base-cased', 'xlnet-large-cased']

data_tags_gt = pickle.load(open(".data2/conll/data_tags.pkl", "rb"))
BI_gt = np.concatenate([np.array(g) for g in data_tags_gt])
num_b = np.count_nonzero(BI_gt == 'B')
print("Num of phrases: ", num_b)
target_names = ['B', 'I']  

ah_distances = pickle.load(open("outputs/run4_test_dists/test_filtered-mean-avg_hellinger-distances.pickle", "rb"))
	...
	for model in model_types:
		...
		for i in range(num_layers): #ith layer
			...
			for row, col in zip(x, y):
					pred[row][col+1] = 'B'
					max_dists.append(mod_vec[row][col])

			threshold.append(np.amin(max_dists))
			...
			pred_path = f'outputs/run5_ratio/{dist_type}/{model}-{dist_type}-{i}.out'
			fc = 0
